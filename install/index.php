...
if (@$_GET['c'] == 'success') {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($data['DB_TYPE'] == 'mysqli') {
            ...
            $link->query("UPDATE $table_admin SET username = '{$username}', password = '{$pass}',salt='{$salt}' , status=1,create_time = '{$create_time}' WHERE id = 1");
            ...
            $version > 5.0 or die("<script>alert('请将您的mysql升级到5.0以上');history.go(-1)</script>");
            mysql_set_charset('utf8', $conn);
            $create_sql = 'CREATE DATABASE IF NOT EXISTS ' . $data['DB_NAME'] . ' DEFAULT CHARACTER SET utf8;';
            mysql_query($create_sql, $conn);
            $selected_db = mysql_select_db($data['DB_NAME'], $conn);
